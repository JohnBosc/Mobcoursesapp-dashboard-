package com.mobcoursesapp.mvc.dao.impl;
...
public class GenericDaoImpl<E> implements IGenericDao<E> {
    ...
    public List<E> selectAll() {
        Query query = em.createQuery("select t from " + type.getSimpleName() + " t"); //JPQL
        return query.getResultList();
    }

    @Override
    public List<E> selectAll(String sortField, String sort) {
        Query query = em.createQuery("select t from " + type.getSimpleName() + " t oder by " + sortField + " " + sort); //JPQL
        return query.getResultList();
    }
    ...
    public E findOne(String paramName, Object paramValue) {
        Query query = em.createQuery("select t from " + type.getSimpleName() + " t where " + paramName + " = :x"); //JPQL
    ...
    public E findOne(String[] paramNames, Object[] paramValues) {
            ...
            for (int i = 0; i < paramValues.length; i++) {
                query.setParameter("x" + i, paramValues);
            }
            return  query.getResultList().size() > 0 ? (E) query.getResultList().get(0) : null;
    }
